...
public class SalasVirtuais extends Controller {
	...
	//???
	public static void salvar(SalaVirtual s, Tabuleiro tabuleiro) {
		if(s.id == null) {
			s.save();
			detalhes(s.id, s);
		}
	...
	public static void deletar(long id, Tabuleiro tabuleiro) {
		SalaVirtual s = SalaVirtual.findById(id);
		//Tabuleiros.deletarTabuleiro(tabuleiro);
		s.delete();
		indexProfessores();
	}
